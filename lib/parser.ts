...
function parseClass(text: string, line: number): ClassNode {
    ...
    if (!m) throw new Error(`Invalid @class declaration: ${text}`);
    const name = m[1];
...
function parseSingle(typeStr: string): TypeNode {
    ...
    if (!str) throw new Error(`Empty type`);
    if (str.endsWith('[]')) {
...
function parseFunctionType(str: string): FunctionTypeNode {
    const headerMatch = /^fun\s*\([^)]*\)/.exec(str.trim());
    if (!headerMatch) throw new Error(`Invalid function type: ${str}`);
    const header = headerMatch[0];
    const tail = str.trim().slice(header.length).trim();
...
function parseTableType(raw: string): TableTypeNode {
    ...
    return {
        kind: 'table', fields: splitTopLevel(content, ',').map(part => {
            ...
            if (!tm) throw new Error(`Invalid table field: ${part}`);
